...
//     MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp.mac-linux.fsproj (unix)
//     MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp.windows.fsproj (windows)
//     MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp.addin.xml
...
let DocFoodFSharpVersion = "0.0.1"
...
if not isWindows then
    ...
    FileReplace ("Makefile", "Makefile", "INSERT_VERSION", DocFoodFSharpVersion)
...
let fsprojFile = "MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp." + tag + ".fsproj"
let xmlFile = "MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp.addin.xml"

//process main project file
FileReplace ("MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp.fsproj.orig", fsprojFile, "INSERT_FSPROJ_MDROOT", mdDir)
...
FileReplace ("MonoDevelop.DocFood.FSharp/MonoDevelop.DocFood.FSharp.addin.xml.orig", xmlFile, "INSERT_FSPROJ_VERSION", DocFoodFSharpVersion)
...
%%MSBUILD%% MonoDevelop.DocFood.FSharp\MonoDevelop.DocFood.FSharp.windows.fsproj /p:Configuration=%s
set MDROOT="%s"
rmdir /s /q pack
mkdir pack\windows\%s
%%MDROOT%%\bin\mdtool.exe setup pack bin\windows\%s\MonoDevelop.DocFood.FSharp -d:pack\windows\%s
%%MDROOT%%\bin\mdtool.exe setup install -y pack\windows\%s\MonoDevelop.DocFood.FSharp_%s.mpack 
"""
           config mdDir config config config config DocFoodFSharpVersion)
